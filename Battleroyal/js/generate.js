function startGame() {
    ...
    for (let i = turn.start; i < turn.end; i++) {
        ...
        document.getElementById('container').innerHTML += `<p id="${players[i].tribute}">${players[i].name}<br><span id=health${i}>Hp: ${players[i].hp}</span><br><span>Type: ${players[i].type.personality}</span><br><span>Weapon: ${players[i].wep.weapon}</span><br><span id="img${i}"></span></p>`;
